...
export function compile(source, { emit = true } = {}) {
  ...
  if (emit) {
    start = performance.now(); ({ wasm, ...emission } = new WasmEmit(ast, inference).run()); emit_ms = performance.now() - start;
...
export function run(source, options = {}) {
  const result = compile(source), execution = execute(result.wasm, options);
